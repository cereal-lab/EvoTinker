...
class DualCandidateSolution(CandidateSolution):
    ...
    def __init__(self, length=None, genotype=None, fitness_evaluator=None):
        ...
        if genotype is None:
            self.genotype = []
            for _ in range(length+1): # <------ DIFFERENCE 
                self.genotype.append(random.randint(0,1))
        else:
            self.genotype = genotype
        self.fitness = None
    ...
    def evaluate(self):
        if self.genotype[0] == 0:
            phenotype = self.genotype[1:]
        else:
            phenotype = [ 1-x for x in self.genotype[1:]]
        self.fitness, self.outcome = self.fitness_evaluator.evaluate(phenotype)
